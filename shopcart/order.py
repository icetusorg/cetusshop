...
from shopcart.models import System_Config, MyUser, Order, Address, Product, Order_Products, Cart_Products, Cart, \
    Product_Attribute, ExpressType, OrderRemark, OrderCustRemark
...
def ajax_cancel_order(request):
    ...
    try:
        ...
        logger.error(
            'Order was not found. Order_id：%s , user_id: %s' % [str(order_to_cancel.order_id), str(request.user.id)])
...
def paypal_notify(sender, **kwargs):
    ...
    if abs(float(ipn_obj.mc_gross) - order.order_amount) > 0.01:
        logger.debug(
            str('总金额校验不正确。订单编号[%s]的总金额为[%s],实际支付金额为[%s]' % [order.order_number, order.order_amount, ipn_obj.mc_gross]))
    ...
    if ipn_obj.receiver_email != paypal_account:
        logger.debug(str('收款账户不正确。订单编号[%s]的实际收款账户为[%s]，配置的收款账户为：[%s]' % [order.order_number, ipn_obj.receiver_email,
                                                                         paypal_account]))
