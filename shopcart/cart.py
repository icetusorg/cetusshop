...
def add_to_cart(request):
        ...
        if add_result_flag:
            for pa in product.attributes.all():
                ...
                if pa.id == product_attribute_id_to_be_add:
                    ...
                    logger.debug(
                        'The min_order_quantity of this product has been changed to :%s' % (min_order_quantity))
                    add_result_flag = True
                    break
        else:
            # 没选特定属性
    ...
    if add_result_flag:
        ...
        if quantity < min_order_quantity:
            ...
            result_dict['message'] = _('The minimun order quantity of the product is %(value)s') % {
                'value': min_order_quantity}
            add_result_flag = False
        if quantity > quantity_can_be_sold:
            result_dict['success'] = False
            result_dict['message'] = _('Sorry, %(value)s pieces are in the stock only.') % {
                'value': quantity_can_be_sold}
            add_result_flag = False
    ...
    if add_result_flag:
        cart_product, create = Cart_Products.objects.get_or_create(cart=cart, product=product,
...
def ajax_modify_cart(request):
    ...
    if 'method' in cart:
        ...
        if cart['method'] == 'add':
            ...
            response['Content-Type'] = "text/javascript"
            response.write(json.dumps(result_dict))
            response.set_cookie('cart_item_type_count', parent_cart.cart_products.all().count(),
                                max_age=3600 * 24 * 365)
            return response
...
def re_calculate_price(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return JsonResponse(ret_dict)
        prices, promotion = get_prices(cart_product_id_list=cart_product_id_list, express_type=express_type,
                                       discount_code=promotion_code)
        if promotion:
...
def calc_shipping_fee(cart_product_id_list, express_type):
    ...
    logger.debug('fix_ship_fee is：%s,weight_ship_fee：%s,stere_ship_fee is:%s' % (
        express_type.price_fixed, weight_ship_fee, stere_ship_fee))
