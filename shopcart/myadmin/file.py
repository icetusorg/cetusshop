...
def get_scale_para(type):
    need_scale = False
    width_limit = 1280
    height_limit = 1280
    try:
        str_need_scale = System_Config.objects.get(name=(type + '_need_scale')).val
        if str_need_scale.upper() == 'TRUE':
            need_scale = True
    except Exception as err:
        logger.info('%s need_scale is not set. Default is False. Message is:%s' % (type, err))

    try:
        str_width_limit = System_Config.objects.get(name=(type + '_width_limit')).val
        width_limit = int(str_width_limit)
    except:
        logger.info('%s width_limit is not set. Default is 1280.' % type)

    try:
        str_height_limit = System_Config.objects.get(name=(type + '_height_limit')).val
        height_limit = int(str_height_limit)
    except:
        logger.info('%s height_limit is not set. Default is 1280.' % type)

    return (need_scale, width_limit, height_limit)
