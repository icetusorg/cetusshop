...
from shopcart.models import System_Config, Product, Product_Images, Category, MyUser, Email, Reset_Password, Address, \
	Product_Attribute, Attribute_Group, Attribute, Article, Express, ExpressType, Order, CustomizeURL, OAuthSite
...
def init_system_configs():
	...
	sys_con = System_Config.objects.create(name='maturity_data',
										   val=(datetime.utcnow() + timedelta(days=365)).strftime("%Y-%m-%d %H:%M:%S"))
										   
										   
	#几个图片自动缩放的参数
	sys_con = System_Config.objects.create(name='product_need_scale', val='TRUE')
	sys_con = System_Config.objects.create(name='article_need_scale', val='TRUE')
	sys_con = System_Config.objects.create(name='slider_need_scale', val='TRUE')
	sys_con = System_Config.objects.create(name='attritube_need_scale', val='FALSE')
	
	sys_con = System_Config.objects.create(name='product_width_limit', val='1200')
	sys_con = System_Config.objects.create(name='product_height_limit', val='1200')
	
	sys_con = System_Config.objects.create(name='article_width_limit', val='1200')
	sys_con = System_Config.objects.create(name='article_height_limit', val='1200')
	
	sys_con = System_Config.objects.create(name='slider_width_limit', val='1200')
	sys_con = System_Config.objects.create(name='slider_height_limit', val='1200')
	
	sys_con = System_Config.objects.create(name='attritube_width_limit', val='36')
	sys_con = System_Config.objects.create(name='attritube_height_limit', val='36')
	
	#允许上传的文件类型
	sys_con = System_Config.objects.create(name='file_upload_allow', val='JPEG,PNG,GIF,JPG')
...
def init_products():
	product_brush = Product(item_number='BRUSH001', name='Top-quality no tangle 360° hair brush ball', quantity=500,
							market_price=14.99, price=9.99, description='',
							short_desc='Contrary to popular belief, Lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old.',
	...
	pa_red = Product_Attribute.objects.create(product=product_brush, sub_item_number=1, quantity=500,
											  price_adjusment=-0.99, image=image_red)
	...
	pa_green = Product_Attribute.objects.create(product=product_brush, sub_item_number=2, quantity=420,
												price_adjusment=1.99, image=image_black)
	...
	pa_blue = Product_Attribute.objects.create(product=product_brush, sub_item_number=3, quantity=360,
											   price_adjusment=0, image=image_red)
...
def init_email():
	...
	email = Email.objects.create(useage='user_registration_success', email_address=email_address, smtp_host=smtp_host,
								 username=username, password=password, template='default',
								 template_file='user_registration_success.html')
	emial_sys = System_Config.objects.create(name='user_registration_success_send_mail', val='true')

	email = Email.objects.create(useage='user_password_modify_applied', email_address=email_address,
								 smtp_host=smtp_host, username=username, password=password, template='default',
								 template_file='user_password_modify_applied.html')
	emial_sys = System_Config.objects.create(name='user_password_modify_applied_send_mail', val='true')

	email = Email.objects.create(useage='user_password_modify_success', email_address=email_address,
								 smtp_host=smtp_host, username=username, password=password, template='default',
								 template_file='user_password_modify_success.html')
	emial_sys = System_Config.objects.create(name='user_password_modify_success_send_mail', val='true')

	email = Email.objects.create(useage='order_was_payed', email_address=email_address, smtp_host=smtp_host,
								 username=username, password=password, template='default',
								 template_file='order_was_payed.html')
