...
//招聘信息提交
jQuery("#recruit-basic-info-submit-btn").click(function (event) {
    event.preventDefault();
    var url = "/admin/recruit-edit/";
    var data = CKEDITOR.instances.recruit_content_editor.getData();
    $("#recruit_content_editor").val(data);
    // alert($("#editor").val())
    $.ajax({
        cache: false,
        type: "POST",
        url: url,
        data: $("#recruit-basic-info-form").serialize(),
        async: false,
        error: function (request) {
            alert("提交失败");
        },
        success: function (data) {
            ...
            if (data.success == true) {
                $('#myModal').on('hidden.bs.modal', function (e) {
                    location.href = url + "?id=" + data.data.recruit_id;
...
//招聘删除
jQuery(".cust-page-recruit").on('click', function (event) {
    var url = "/admin/recruit-";
    method = $(this).data("method");
    url = url + method + "/";
    console.log("url:" + url);
...
jQuery(".sku_delete_link").click(function (event) {
    ...
    var url = "/admin/product-sku-delete/delete/" + $(this).data("sku-id") + "/";
...
//清除SKU
jQuery("#product-attribute-clear-btn").click(function (event) {
    event.preventDefault();//阻止A标签跳转

    var url = "/admin/product-sku-delete/clear/" + $(this).data("product-id") + "/";
...
jQuery("#article_main_picture_show2").on('click', ".set-picture-attr", function (event) {
    event.preventDefault();
    var img_url = $(this).data("image-url");
    var type = $(this).data("image-type");
    var article_id = $(this).data("article-id");
...
jQuery("#product_main_picture_show2").on('click', ".set-picture-attr", function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#infoMessage").html(data.message);
            $("#myModal").modal('toggle');
            reload_picture_list_attachment();
        }
    });
});
