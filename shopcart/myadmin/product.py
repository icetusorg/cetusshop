...
from shopcart.models import Product, System_Config, Category, Attribute, Attribute_Group, Product_Attribute, \
    Product_Images, ProductParaGroup, ProductPara, ProductPrice, ProductParaDetail, Album, ProductImportError, \
    BatchTaskMonitor
from shopcart.forms import product_add_form, product_basic_info_form, product_detail_info_form, product_para_group_form, \
    product_sku_group_form
...
def product_sku_group_edit(request):
    ...
    if request.method == 'GET':
        ...
        return TemplateResponse(request, System_Config.get_template_name('admin') + '/product_sku_group_detail.html',
                                ctx)
...
def product_para_list(request):
    ...
    if request.method == 'GET':
        ...
        page_size = 12
        product_para_list, page_range, current_page = my_pagination(request=request, queryset=product_para_list,
                                                                    display_amount=page_size)
        ...
        ctx['item_count'] = ProductParaGroup.objects.all().count()
        ctx['page_size'] = page_size
        return TemplateResponse(request, System_Config.get_template_name('admin') + '/product_para_list_content.html',
                                ctx)
    else:
        raise Http404
...
def product_sku_manage(request, id=None):
    ...
    if request.method == 'POST':
        ...
        for sku_item in sku:
            ...
            if not is_exist:
                pa = Product_Attribute.objects.create(product=product, sub_item_number=str(sub_item_number), quantity=0,
                                                      price_adjusment=0.0, image=None, min_order_quantity=1)
